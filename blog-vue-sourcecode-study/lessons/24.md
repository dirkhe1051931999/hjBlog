...
2. parseHtml 过程是利用正则表达式解析模板，解析出开始标签，闭合标签，文本
3. parseHtml 的回调中，扩展解析内容，比如标记 type，parent 指向，拿到 v-if v-for，根据分隔符{{}}取出表达式等
4. AST 有三种类型，type 为 1 的是普通元素，type 为 2 是表达式，type3 是纯文本
