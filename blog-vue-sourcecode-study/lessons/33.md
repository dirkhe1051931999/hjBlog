...
- action
  > 1. 因为 mutation 必须是同步函数，如果需要一部更改 state，store 中有一个 actions，提供了异步更改 state
...
2. vuex 是的 store 是如何注入的
   > 通过 mixin 的方式把 store 注入到 vue 上传文的$store中，在组件中可以使用this.$store 访问到 store
...
6. vuex 有何“缺陷”
   > 1. actions 不易理解，刚接触 vuex 会任务 actions 和 mutations 更改 state 的原理一致，实力上 actions 最终走的还是 mutations，容易产生误导
   > 2. vuex 推荐使用 mutation 修改 state，但是除 mutation 外，在非严格模式，还可以直接绕开 mutations 修改 state，在 vue 中通过 this.\$store.state.[name]更改 state，在工作中，我也发现很多人这样做
