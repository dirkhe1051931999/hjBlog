...
> 在初始化 computed 的时候，会触发渲染 watcher 订阅 computed ，然后在求值的过程中，会执行计算属性的 getter 函数，又因为求值的两个变量是响应式对象，所以又触发它俩的 getter，把它俩自身持有 dep 添加到 watcher 中，然后对比计算后的新值和旧值，只有最终计算的值发生变化，才会执行 this.dep.notify()，触发渲染 watcher 重新渲染
