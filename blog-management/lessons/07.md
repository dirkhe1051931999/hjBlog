...
- 为什么要定时向 mysql 写入数据呢，具体个例子，常见的点赞，取消点赞的高频次操作，每次都在读写数据库，大量的操作肯定会影响数据库的性能，所以就需要做缓存，缓存就是先放在 redis 中，比如每天晚上 12 点，把点赞的数据存入 mysql 中，12 点的点赞与点赞数据查询都在 redis 中完成。
